...
BOOST_FIXTURE_TEST_CASE(simple_qubit_to_qutrit_eiH, qubit_to_qutrit_fixture_DEBUG<RandHermExp>)
{
  MyStatsCollector stats;
  typedef Tomographer::MHRandomWalk<std::mt19937, ChannelSpaceMHWalkerType, MyStatsCollector,
                                    Tomographer::MHRWNoController,
                                    Tomographer::Logger::BoostTestLogger> MHRandomWalk;
  Tomographer::MHRWNoController noctrl;
  MHRandomWalk rw(0.1, 10, 500, 1024, mhwalker, stats, noctrl, rng, logger);
  rw.run();

  BOOST_CHECK( true ) ; // dummy test case -- for now, just check that the above compiles & runs
}
BOOST_FIXTURE_TEST_CASE(simple_qubit_to_qutrit_elr, qubit_to_qutrit_fixture_DEBUG<ElemRotations>)
{
  MyStatsCollector stats;
  typedef Tomographer::MHRandomWalk<std::mt19937, ChannelSpaceMHWalkerType, MyStatsCollector,
                                    Tomographer::MHRWNoController,
...
BOOST_FIXTURE_TEST_CASE(jump_fn_symmetric_eiH, qubit_to_qutrit_fixture_DEBUG<RandHermExp>)
{
  ...
  typedef ChannelSpaceMHWalkerType::PointType PointType;
  ...
  for (int k = 0; k < N; ++k) {
    ...
    totalV += newV;
    if (k % 20000 == 0) {
      BOOST_TEST_MESSAGE("k = " << k << "/" << N << " --> " << (k*100.0/N) << "%") ;
    }
  }
